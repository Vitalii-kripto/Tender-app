...
class AiService:
    ...
    def extract_structured_tender_data(self, text: str):
        """Серверное структурированное извлечение ключевых данных по предмету закупки из всех документов тендера"""
        if not self.client:
            return {}

        logger.info(f"Extracting structured tender data. Context length: {len(text)}")
        prompt = f"""
        Роль: Аналитик тендерной документации.
        Задача: Выполни серверное структурированное извлечение ключевых данных по предмету закупки из всех предоставленных документов тендера.
        
        ТРЕБОВАНИЯ К ИЗВЛЕЧЕНИЮ:
        1. Извлеки наименование каждой позиции, единицу измерения, количество, характеристики, цену за единицу (если есть).
        2. Извлеки общую НМЦК.
        3. Извлеки сроки поставки.
        4. Извлеки условия доставки, разгрузки и перемещения.
        5. Извлеки ограничения по упаковке, маркировке, транспорту и приемке.
        6. Извлеки сведения о заказчике (наименование, ИНН, адрес, телефон, email, контактное лицо).
        7. Извлеки возможность поставки эквивалентов/аналогов (разрешены ли, есть ли запрет на слова "эквивалент"/"аналог", требуются ли точные показатели).
        8. Если параметр найден хотя бы в одном документе, он считается найденным для всего тендера.
        ...
        {{
            ...
            "items": [
                {{
                    ...
                    "quantity": 0.0,
                    "price_per_unit": 0.0,
                    "characteristics": "Характеристики по позиции"
                }}
            ],
            "delivery_terms": "Сроки поставки",
            "logistics": "Условия доставки, разгрузки и перемещения",
            "restrictions": "Ограничения по упаковке, маркировке, транспорту и приемке",
            "equivalents": {{
                "is_allowed": true,
                "exact_indicators_required": false,
                "conditions": "Условия допуска эквивалентов или обоснование запрета",
