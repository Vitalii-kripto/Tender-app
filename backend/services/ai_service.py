...
class AiService:
    ...
    def search_products_internet(self, query: str):
        ...
        # Промпт усилен для поиска реальных данных
        prompt = f"""
        ЗАДАЧА: Найти РЕАЛЬНЫЕ гидроизоляционные материалы, доступные для покупки в РФ, соответствующие запросу: "{query}".
        
        ОБЯЗАТЕЛЬНО ИСПОЛЬЗУЙ GOOGLE SEARCH. Не выдумывай товары.
        Ищи на сайтах: technonicol.ru, gidroizol.ru, petrovich.ru и других поставщиков.
        
        Формат ответа (Markdown):
        Для каждого найденного товара (найди 2-3 варианта):
        
        1. **Название товара** (Производитель)
        2. *Характеристики*: Основа, Толщина, Гибкость, Вес.
        3. *Цена*: Актуальная цена за м2 или рулон (укажи источник или дату).
        4. *Где купить*: Название магазина или ссылка (если есть).
        
        Если ничего не найдено, напиши "К сожалению, прямых аналогов в открытом доступе не найдено".
        """
